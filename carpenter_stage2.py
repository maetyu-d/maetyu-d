...
def shimmer(send_stereo, amount=0.15):
    ...
    cloud = multitap_cloud(up, mix=0.6)
    return cloud * amount

# ====== Arrangement (Stage 2 – Tension Rise) ======
...
pad = one_pole_lp(pad, 1200.0, SR) * np.linspace(0,1,L) * 0.25
...
# Light vintage shimmer on the pad+lead bus
send = P*0.6 + lead*0.8
audio += shimmer(send, amount=0.15)

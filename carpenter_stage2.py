...
def mtap(x,del_ms=(90,140,200,260,340),g=(.28,.23,.19,.16,.12),mix=.35,pre=28):
    ...
    for dms,gg in zip(del_ms,g):
        d=pre+int(dms*SR/1000); pad=np.zeros((d,2)); acc+=gg*np.vstack([pad,x[:-d]])
    return out+mix*acc
def pitch_up(sig):
    if sig.ndim==1: sig=np.column_stack([sig,sig]); N=sig.shape[0]
    idx=np.linspace(0,N-1,num=max(1,N//2)); fast=np.zeros((idx.size,2)); t=np.arange(N)
...
def shimmer(x,amt=.16):
    hp=np.column_stack([hpf_ma(x[:,0],max(3,int(SR/1200))),hpf_ma(x[:,1],max(3,int(SR/1200)))])
    up=pitch_up(hp); cloud=mtap(up,mix=.6); return cloud*amt
A2,G2,F2,E2,A3,C4,D4,E4,G4,A4=110.0,98.0,87.31,82.41,220.0,261.63,293.66,329.63,392.0,440.0
pat=[A2,A2,A2,A2,G2,G2,A2,A2,F2,F2,F2,F2,E2,E2,E2,E2]; gate=int(0.9*eighth)
L=int(SPB*4*BARS); audio=np.zeros((L,2))
# kick
for b in range(int(L/SPB)):
    ...
    s=lpf_ma(s*np.linspace(1,0,N),max(3,int(SR/3000))); s=softclip(s,2)*0.56; audio[n0:n1,0]+=s; audio[n0:n1,1]+=s
# bass
for s in range(L//eighth):
    n0=s*eighth; n1=min(L,n0+eighth); seg=n1-n0; f=pat[s%len(pat)]; g=min(gate,seg)
    o=0.6*saw(f,seg)+0.4*sqr(f*1.005,seg,0.48); e=env(seg,g,5,80,.7,60); l=o*e; r=l if seg<=24 else np.concatenate([np.zeros(12),l[:-12]])
    l=softclip(l*np.linspace(.55,1.0,L)[n0:n1],1.7); r=softclip(r*np.linspace(.55,1.0,L)[n0:n1],1.7)
...
for b in range(0,int(L/SPB),4):
    ...
    s=0.8*hpf_ma(nz,max(3,int(SR/1800)))+0.5*bd; s=softclip(s,1.4)*0.5
    audio[n0:n1,0]+=s*0.9; audio[n0:n1,1]+=np.concatenate([np.zeros(2),s[:-2]]) if N>2 else s
# hats
for i in range(int(L/SPB)*2):
    if np.random.rand()>0.95: continue
    n0=int(i*eighth); n1=min(L,n0+int(0.05*SR)); N=n1-n0
    nz=np.random.uniform(-1,1,N); s=hpf_ma(nz,max(3,int(SR/6000)))*np.linspace(1,0,N); s=softclip(s,1.1)*0.17
    audio[n0:n1,0]+=s*0.7; audio[n0:n1,1]+=s
# pad + lead
pad=0.6*(saw(E4,L)+saw(E4*0.993,L,np.pi/3)); pad=lpf_ma(pad,24)*np.linspace(0,1,L)*0.25
P=np.column_stack([pad,np.concatenate([np.zeros(10),pad[:-10]])]); audio+=P
motif=[(1.0,E4,0.75),(2.0,87.31*4,0.5),(3.0,440.0,0.5),(5.0,392.0,0.5),(5.5,440.0,0.5),(6.0,329.63,0.75),
...
for beat,fq,dur in motif:
    ...
    b=0.55*sqr(fq,N,0.48)+0.45*saw(fq*0.997,N); e=env(N,int(0.85*seg),8,120,0.7,200); s=lpf_ma(b,14)*e; s=softclip(s,1.2)*0.30
    if int(beat)%2==0: lead[n0:n1,0]+=s; lead[n0:n1,1]+=s*0.7
    else: lead[n0:n1,0]+=s*0.7; lead[n0:n1,1]+=s
audio+=lead
# shimmer (light)
send=P*0.6+lead*0.8; audio+=shimmer(send,0.15)
audio=softclip(audio,1.15); audio=norm(audio,0.97)
